...
async function createR2Client(): Promise<S3Client | null> {
...
async function listR2Files(client: S3Client): Promise<R2File[]> {
      ...
      if (response.Contents) {
        response.Contents.forEach((object) => {
          if (object.Key && object.Size !== undefined && object.LastModified) {
            ...
            const mediaAssetId = key.replace("media-assets/", "").replace(".mp3", "");
...
async function getR2FileMetadata(
  client: S3Client,
  key: string
...
async function main() {
  ...
  try {
    ...
    for (const file of r2Files) {
      if (!file.mediaAssetId || file.mediaAssetId === "test-upload/tere-bin-test") {
      ...
      if (dbRecord) {
        if (dbRecord.status === "READY" && dbRecord.storageKey === `media-assets/${file.mediaAssetId}.mp3`) {
          matchedCount++;
    ...
    console.log(`\n✓ Matched: ${matchedCount}/${r2Files.length - 1}`);
    ...
    const orphanedRecords = readyRecords.filter(
      (record) => record.storageKey && !r2FileIds.has(record.storageKey.replace("media-assets/", "").replace(".mp3", ""))
    );

    if (orphanedRecords.length === 0) {
      console.log("✓ All READY records have corresponding R2 files");
    } else {
      console.log(`❌ Found ${orphanedRecords.length} READY records without R2 files:\n`);
    ...
    const avgFileSize = r2Files.length > 0 ? (totalSize / r2Files.length / 1024).toFixed(2) : 0;
