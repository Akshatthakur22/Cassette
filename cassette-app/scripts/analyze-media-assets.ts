...
async function main() {
  ...
  try {
    ...
    statusCounts.forEach((sc) => {
      const bar = "█".repeat(Math.round((sc.count / Math.max(1, totalCount)) * 40));
      console.log(
        `${sc.status.padEnd(12)} │ ${String(sc.count).padStart(4)} │ ${String(sc.percentage).padStart(3)}% │ ${bar}`
      );
    });
    ...
    readyAssets.forEach((asset, idx) => {
      ...
      console.log(
        `${String(idx + 1).padStart(2)}. [${asset.id}]`
      );
      ...
      console.log(
        `    Completed: ${asset.processedAt ? new Date(asset.processedAt).toISOString() : "N/A"}`
      );
      console.log("");
    });
    ...
    sortedErrors.forEach((group, idx) => {
      ...
      group.examples.forEach((ex) => {
        console.log(`    - "${ex.title}" (${ex.videoId}, attempts: ${ex.attemptCount})`);
      });
      console.log("");
    });
    ...
    // Check for duplicate videos (same providerTrackId, different status)
    const duplicateCheckResults = await prisma.$queryRaw`
      SELECT provider, providerTrackId, COUNT(*) as count, GROUP_CONCAT(id) as ids
      FROM MediaAsset
      GROUP BY provider, providerTrackId
      HAVING count > 1
    ` as Array<{ provider: string; providerTrackId: string; count: number; ids: string }>;

    console.log(
      `✓ Duplicate videos (same source ID, multiple records): ${duplicateCheckResults.length || 0}`
    );

    if (duplicateCheckResults.length > 0) {
      duplicateCheckResults.forEach((dup) => {
        console.log(
          `    ${dup.provider}:${dup.providerTrackId} has ${dup.count} records: ${dup.ids}`
        );
      });
    }
    ...
    const successRate =
      totalCount > 0
        ? Math.round((readyAssets.length / totalCount) * 100)
        : 0;
    ...
    console.log(`✓ Failure Rate: ${failureRate}% (${failedAssets.length}/${totalCount} failed)`);
    console.log(
      `✓ Processing Rate: ${processingAssets.length} assets in-progress`
    );
    console.log(
      `✓ Retry Eligible: ${failedAssets.filter((a) => a.attemptCount < 5).length} FAILED assets can be retried`
    );
